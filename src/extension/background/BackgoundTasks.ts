...
import {
  sendGreenNotificationAndExtensionIcon,
  sendYellowNotificationAndExtensionIcon
} from "@/core/service/notifications";
...
export class BackgroundTasks {
  ...
  verifyCanistersOnPageLoad() {
    ...
    const listener = async details => {
      ...
      const isCurrentUrlRelatedToCanisters = Object.keys(urlsVsCanisters).includes(cleanedUrl)
      if (!isChromeExtensionUrl(cleanedUrl)) {
        if (tabToVerify.isEnabled === false && isCurrentUrlRelatedToCanisters && tabToVerify.url !== cleanedUrl) {
  ...
  async persistDetectedCanisters(details: WebRequest.OnSendHeadersDetailsType) {
    ...
    if (details.url && !isChromeExtensionUrl(details.url)) {
      ...
      canisterIds.forEach(id => {
            this.backgroundStore.dispatch(
                persistDetectedCanister({
                  ...
                  url: cleanUrl(details.initiator)
                }))
          }
      )
    }
  ...
  async createTabToVerifyCanister(tabId: number, url: string) {
    this.backgroundStore.dispatch(enableTabToVerify({tabId: tabId.toString(), url}))
  ...
  async notifyUserAboutCanistersState() {
    const {urlsVsCanisters} = this.backgroundStore.getState().detectedCanisterReducer
    const {coverInfo} = this.backgroundStore.getState().coverInfoReducer
    const {tabToVerify} = this.backgroundStore.getState().detectedCanisterReducer
