...
import { ContentScriptMessages } from "@/core/messages";

const Verification = () => {
  const canisterIds = useAppSelector((state) =>
    Object.keys(state.detectedCanisterReducer.detectedCanisters)
  );
  ...
  const fetchCanisterData = useCallback(async () => {
    const isCoverVerified = (id) =>
      coverInfo[id] !== undefined && coverInfo[id].isLoading === false;

    const coverVerifiedIds: string[] =
      Object.keys(coverInfo).filter(isCoverVerified);

    if (canisterIds.length === coverVerifiedIds.length) {
      await browser.runtime.sendMessage({
        type: ContentScriptMessages.CANISTERS_CHECK_COMPLETE,
      });
      window.close();
    } else {
